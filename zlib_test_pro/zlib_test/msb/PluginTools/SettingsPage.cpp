...
SettingsPage::~SettingsPage() {
	...
	for( unsigned int i = 0; i < m_checks.size(); i++ ) {
        ...
        if( item->getCall() == nullptr && strcmp( item->getLongName(), "_dummy_" ) == 0 ) {
			delete item;
		}
	}
}
...
//                SettingsPage* subPage = ( item == nullptr ? nullptr : item->getSettingsPage() );
//                if( subPage != nullptr ) {
...
//                    if( subPage != nullptr && settings.hasSubParameterValues( param->getName(), tmp ) ) {
		...
		if( afterBslash ) {
			...
			if( c == '[' ) {
                ...
                if( param != nullptr ) {
                    ...
                    if( val != nullptr && val->getEnumType() == SettingsValue::ENUM_PLUGIN ) {
						int idx = -1;
