...
PluginMenu::PluginMenu() {
    ...
    m_execCall = nullptr;
    m_enableCall = nullptr;
    m_checkedCall = nullptr;
...
void PluginMenu::setGroup( const char* name, int order ) {
    ...
    GroupOrder* gOrder = nullptr;
    for( unsigned int i = 0; i < m_groupOrders.size() && gOrder == nullptr; i++ ) {
...
PluginMenu* PluginMenu::getChildMenu( unsigned int index ) {
    ...
    if( m_children == nullptr || index >= m_children->size() ) {
        return nullptr;
	}
...
PluginMenu* PluginMenu::getChildMenu( const char* name ) {
	...
	// check parameter and member
    if( m_children == nullptr || nameStr.empty() ) {
        return nullptr;
	}
...
bool PluginMenu::isEnabled( kome::objects::Variant& acObj ) {
	...
	case TARGET_SAMPLE:
		...
		break;
	case TARGET_GROUP:		
        enabled = ( group != nullptr && group->getNumberOfSpectra() > 0 );
		break;
	case TARGET_SPECTRUM:
        enabled = ( spec != nullptr );
		break;
	case TARGET_CHROMATOGRAM:
        enabled = ( chrom != nullptr );
		break;
	case TARGET_DATAMAP:
        enabled = ( dm != nullptr );
		break;
    case TARGET_NOT_NULL:
        if( dm != nullptr || spec != nullptr || chrom != nullptr ){
			enabled = true;
		}
		break;
		
	case TARGET_SPEC_CHROM:
        if( spec != nullptr || chrom != nullptr ) {
			enabled = true;
		}
		break;
	...
	// get call
    if( enabled && m_enableCall != nullptr ) {
		kome::objects::Parameters params;
...
bool PluginMenu::isChecked( kome::objects::Variant& acObj ) {
	// check members
    if( m_checkedStatusName.empty() && m_checkedCall == nullptr ) {
		return false;
	}
...
bool PluginMenu::isCheckable() {
    return ( m_checkedCall != nullptr || !m_checkedStatusName.empty() );
}
