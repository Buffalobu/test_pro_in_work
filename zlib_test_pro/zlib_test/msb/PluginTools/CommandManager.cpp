...
//	// value
//    kome::plugin::SettingsParam* param = ( form == nullptr ? nullptr : form->getParam() );
//    SettingsValue* value = ( param == nullptr ? nullptr : param->getValue() );
//    if( value == nullptr ) {
//		return;
//	}
...
//	// set parameters
//    ParameterInfo* optionParam = nullptr;
//	std::string itemType;
...
//	if( !itemType.empty() ) {
//        for( unsigned int i = 0; i < m_optionParams.size() && optionParam == nullptr; i++ ) {
...
//            if( page != nullptr && page->getNumberOfForms() > 0 ) {
...
//    if( optionParam != nullptr && optionParam->items.size() > 0 ) {
...
std::string CommandManager::getParameterUsage( const char* value ) {
	...
	for( unsigned int i = 0; i < m_optionParams.size(); i++ ) {
        ...
        for( unsigned int j = 0; j < m_optionParams[ i ].items.size() && item == nullptr; j++ ) {
        ...
        if( item != nullptr ) {
			...
			for( unsigned int j = 0; j < page->getNumberOfForms(); j++ ) {
                ...
                paramUsage.append( param == nullptr ? "" : param->getName() );
			}
...
// <<<<<<	@Date:2013/10/03	<Add>	A.Ozaki
	...
	for( int i = 1; i < argc && result; i++ ) {
		...
		// get function call
		char* arg = argv[ i ];
        OptionInfo* o = nullptr;
		if( skipSet.find( i ) == skipSet.end() ) {
			o = getOptionInfo( arg );
            if( o == nullptr ) {   // invalid option
        ...
        if( o != nullptr ) {
			...
			// get parameters
            SettingsPage* page = ( o->item == nullptr ? nullptr : o->item->getSettingsPage() );
...
CommandManager::OptionInfo* CommandManager::getOptionInfo( char* arg ) {
	...
	// get option information
    OptionInfo* oi = nullptr;
	bool optionFound = false;

    for( unsigned int i = 0; i < m_commandOptions.size() && oi == nullptr; i++ ) {
		OptionInfo& o = m_commandOptions[ i ];
