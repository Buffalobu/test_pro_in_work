...
kome::objects::SampleSet* FileFactory::openFile( const char* path, kome::core::Progress* progress ) {	// add param @date 2014.0.08 <Mod> M.Izumi 
    kome::objects::SampleSet* ss = nullptr;
...
kome::objects::SampleSet* FileFactory::onOpenFile( const char* path, kome::core::Progress* progress ) {		// add param @date 2014.0.08 <Mod> M.Izumi 
	...
	int cnt = 0;
	createFunctionArrays();
    for( unsigned int i = 0; i < m_openFunctions.size() && sampleSet == nullptr; i++ ) {
		// open information
		...
		// check supportted
        if( supported && info->supportedCall != nullptr ) {
			// parameters
...
bool FileFactory::lessOpenInfo( const OpenInfo& oi0, const OpenInfo& oi1 ) {
	// check open function
    if( oi0.openCall != nullptr && oi1.openCall == nullptr ) {
		return true;
	}
    else if( oi0.openCall == nullptr && oi1.openCall != nullptr ) {
		return false;
	}

	// check support function
    if( oi0.supportedCall != nullptr && oi1.supportedCall == nullptr ) {
		return true;
	}
    else if( oi0.supportedCall == nullptr && oi1.supportedCall != nullptr ) {
		return false;
	}
