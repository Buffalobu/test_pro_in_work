...
{
    if  ( (char *)nullptr == pcPassword || (char *)nullptr == pcDecrypt )
	{
		return	false;
	}
...
{
    if  ( (char *)nullptr == pcPassword || (char *)nullptr == pcEncrypt )
	{
		return	false;
	}
...
bool IniFile::load( const char* path ) {
    ...
    Properties* params = nullptr;
	std::string section;

    while( fgets( line, INI_LINE_BUFFER_SIZE, fp ) != nullptr ) {	// read line
		// read line
...
IniFile* IniFile::loadIniFile( const char* path ) { 
    ...
    if( iniFile == nullptr ) {	// create new object
		iniFile = new IniFile();
