...
DataPoints::DataPoints( ArrayType type ) : m_type( type ) {
	// initialize
	if( type == FLOAT ) {
        m_xArray.fltarry = nullptr;
        m_yArray.fltarry = nullptr;
	}
	else{
        m_xArray.dblarry = nullptr;
        m_yArray.dblarry = nullptr;
	}
...
void DataPoints::onClearPoints() {
	// delete array
	if( m_type == FLOAT ) {
        if( m_xArray.fltarry != nullptr ) {
			delete[] m_xArray.fltarry;
		}
        if( m_yArray.fltarry != nullptr ) {
        ...
        if( m_xArray.dblarry != nullptr ) {
			delete[] m_xArray.dblarry;
		}
        if( m_yArray.dblarry != nullptr ) {
