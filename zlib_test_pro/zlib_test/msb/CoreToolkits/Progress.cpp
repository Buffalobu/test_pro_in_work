...
//	boost::this_thread::sleep( boost::posix_time::milliseconds( FILL_INTERVAL ) );
    std::this_thread::sleep_for (std::chrono::milliseconds(FILL_INTERVAL));
#endif
//
// <<<<<<	@Date:2013/11/18	<Add>	A.Ozaki
...
Timer* Progress::createTimer( const char* name ) {
	// check the name
    if( name == nullptr || m_timers == nullptr ) {
        return nullptr;
	}
...
const char* Progress::getTimerName( const unsigned int index ) {
    if( m_timers == nullptr || index >= m_timers->size() ) {
        return nullptr;
	}
...
// constructor
IgnoringProgress::IgnoringProgress() : Progress( nullptr ) {
}
...
// constructor
SubProgress::SubProgress( Progress& parent ) : Progress( nullptr ) {
	m_parent = &parent;
}
...
{
    m_parent->setStatus( (char *)nullptr, true );
	return;
}
//
// <<<<<<	@Date:2013/11/25	<Add>	A.Ozaki
