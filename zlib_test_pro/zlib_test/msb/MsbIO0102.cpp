/**
 * @file MsbIO0102.cpp
 * @brief implements of MsbIO0102 class
 *
 * @author S.Tanaka
 * @date 2006.10.16
 * 
...
// static members
MsbManager::DataValue MsbIO0102::m_fileHeader[] = {
...
MsbManager::DataValue MsbIO0102::m_dataHeader[] = {
...
MsbManager::DataValue MsbIO0102::m_groupInfo[] = {
...
MsbManager::DataValue MsbIO0102::m_spectrumInfo[] = {
	...
	DataValue( TYPE_ULONG,  (unsigned int)0 ),		 //  8: Scan Number
	DataValue( TYPE_CHAR,   (char)0 ),				  //  9: Polarity
	DataValue( TYPE_STRING, (MsbString)0 ),			 // 10: Title
	DataValue( TYPE_BOOL,   (bool)false ),			  // 11: Chromatogram Flag
	DataValue( TYPE_STRING, (MsbString)0 )			  // 12: Icon Name
};

MsbManager::DataValue MsbIO0102::m_partInfo[] = {
...
// create string table
void MsbIO0102::createStringTable( char** stringTable ) {
	// get size
...
void MsbIO0102::getFileHeaderInfo( MsbSample& sample, char* stringTable ) {
	// add file info
...
// read spectrum group
void MsbIO0102::readDataGroupNodes(
...
void MsbIO0102::readSpectra(
	...
	for( unsigned int i = 0; i < length; i++ ) {
		...
		spec->setScanNumber( m_spectrumInfo[ 8 ].val.ul );

		char polarity = m_spectrumInfo[ 9 ].val.c;
		if( polarity < 0 ) {
		...
		spec->setTitle( stringTable + m_spectrumInfo[ 10 ].val.ms );
		spec->setHasChromatogram( m_spectrumInfo[ 11 ].val.b );
		spec->setIcon( stringTable + m_spectrumInfo[ 12 ].val.ms );
...
// read spectrum part
void MsbIO0102::readSpectrumParts(
		std::map< unsigned int, MsbSpectrum* >& specMap
){
	// read header
	READ_FROM_FILE( m_dataHeader );
...
void MsbIO0102::readSpectrumProperties(
		std::map< unsigned int, MsbSpectrum* >& specMap,
		char* stringTable
...
// read file property
void MsbIO0102::readFileProperties( MsbSample& sample, char* stringTable ) {
	// read header
	READ_FROM_FILE( m_dataHeader );
...
void MsbIO0102::writeDataGroupNodes(
...
// write spectrum data
void MsbIO0102::writeSpectrumData(
		unsigned int id,
...
void MsbIO0102::writeSpectra(
	...
	for( unsigned int i = 0; i < dataSet.getNumberOfSpectra(); i++ ) {
		...
		m_spectrumInfo[ 8 ].val.ul  = spec->getScanNumber();				  // Scan Number
		if( spec->getPolarity() == kome::objects::Spectrum::POLARITY_NEGATIVE ) {
			m_spectrumInfo[ 9 ].val.c = -1;
		}
		else if( spec->getPolarity() == kome::objects::Spectrum::POLARITY_POSITIVE ) {
			m_spectrumInfo[ 9 ].val.c = 1;
		}
		else {
			m_spectrumInfo[ 9 ].val.c = 0;
		}																	 // Polarity
		m_spectrumInfo[ 10 ].val.ms  = st( spec->getTitle() );				// Title
		m_spectrumInfo[ 11 ].val.b   = spec->hasChromatogram();			   // Chromatogram Flag
		m_spectrumInfo[ 12 ].val.ms = st( spec->getIcon() );				  // Icon Name
...
void MsbIO0102::writeSpectrumParts( std::vector< SpectrumPartInfo >& parts ) {
...
void MsbIO0102::writeSpectrumProperties(
		kome::objects::DataSet& dataSet,
		std::map< kome::objects::Spectrum*, unsigned int >& spectrumMap
) {
	// get properties
...
// write file property
void MsbIO0102::writeFileProperties( kome::objects::Sample& sample ) {
	// get properties
...
void MsbIO0102::onReadSampleInfo( MsbSample& sample ) {
	// read file header
...
// open sample
void MsbIO0102::onReadMsb( MsbSample& sample ) {
	// create string table
	char* stringTable = NULL;
	createStringTable( &stringTable );
...
bool MsbIO0102::onWriteMsb(
	...
	// log
	LOG_INFO( FMT( "Writing MSB format sample. [ver 1.2]" ) );
