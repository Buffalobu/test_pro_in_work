/**
 * @file MsbIO0108.cpp
 * @brief implements of MsbIO0108 class
 *
 * @author S.Tanaka
 * @date 2008.02.06
 * 
...
// static members
MsbManager::DataValue MsbIO0108::m_fileHeader[] = {
	...
	DataValue( TYPE_USHORT, (unsigned short)8 ),				 //  3: Minor Version
	...
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 //  9: Spectrum Data Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 10: Spectrum Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 11: Spectrum Part Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 12: Spectrum Properties Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 13: String Table Align
	DataValue( TYPE_ULONG,  (unsigned int)0 ),				  // 14: Compressed String Table Size
	DataValue( TYPE_ULONG,  (unsigned int)0 ),				  // 15: Uncompressed String Table Size
	DataValue( TYPE_TIME,   (time_t)0 )						  // 16: Time
};

MsbManager::DataValue MsbIO0108::m_dataHeader[] = {
...
MsbManager::DataValue MsbIO0108::m_groupInfo[] = {
...
MsbManager::DataValue MsbIO0108::m_spectrumInfo[] = {
...
MsbManager::DataValue MsbIO0108::m_partInfo[] = {
...
MsbManager::DataValue MsbIO0108::m_prop[] = {
	DataValue( TYPE_ULONG,  (unsigned int)0 ),	  //  0: Spectrum
...
// create string table
void MsbIO0108::createStringTable( char** stringTable ) {
	// get size
	unsigned long stringTableSize = m_fileHeader[ 15 ].val.ul;
	...
	*stringTable = st;
	...
	// create buff
	unsigned int buffSize = m_fileHeader[ 14 ].val.ul;
	unsigned char* buff = new unsigned char[ buffSize ];

	// read string table
	fileseek( m_fp, m_fileHeader[ 13 ].val.ull, SEEK_SET );
...
void MsbIO0108::getFileHeaderInfo( MsbSample& sample, char* stringTable ) {
	// add file info
...
// read spectrum group
void MsbIO0108::readDataGroupNodes(
...
void MsbIO0108::readDataGroupNodeProperties(
		std::map< unsigned int, kome::objects::DataGroupNode* >& groupMap,
		char* stringTable
) {
   // read header
...
// read spectrum part
void MsbIO0108::readSpectrumParts(
		std::map< unsigned int, MsbSpectrum* >& specMap
){
	// read header
	READ_FROM_FILE( m_dataHeader );
...
void MsbIO0108::readSpectrumProperties(
		std::map< unsigned int, MsbSpectrum* >& specMap,
		char* stringTable
...
void MsbIO0108::writeDataGroupNodes(
...
void MsbIO0108::writeDataGroupNodeProperties(
		std::vector< kome::objects::DataGroupNode* >& groups,
		std::map< kome::objects::DataGroupNode*, unsigned int >& groupMap
) {
	// get properties
...
// write spectrum data
void MsbIO0108::writeSpectrumData(
		unsigned int id,
...
void MsbIO0108::writeSpectrumParts( std::vector< SpectrumPartInfo >& parts ) {
...
void MsbIO0108::writeSpectrumProperties(
		kome::objects::DataSet& dataSet,
		std::map< kome::objects::Spectrum*, unsigned int >& spectrumMap
) {
	// get properties
...
void MsbIO0108::onReadSampleInfo( MsbSample& sample ) {
	// read file header
...
// open sample
void MsbIO0108::onReadMsb( MsbSample& sample ) {
	// create string table
	char* stringTable = NULL;
	createStringTable( &stringTable );
	...
	unsigned long long spectrumAlign	  = m_fileHeader[ 10 ].val.ull;
	unsigned long long specPartAlign	  = m_fileHeader[ 11 ].val.ull;
	unsigned long long specPropAlign	  = m_fileHeader[ 12 ].val.ull;
...
bool MsbIO0108::onWriteMsb(
	...
	// log
	LOG_INFO( FMT( "Writing MSB format sample. [ver 1.8]" ) );
	...
	m_fileHeader[ 16 ].val.t = time( NULL );					// Time
	...
	m_fileHeader[ 9 ].val.ull = filetell( m_fp );   // Spectrum Data Align
	...
	m_fileHeader[ 10 ].val.ull = filetell( m_fp );					  // Spectrum Align
	...
	m_fileHeader[ 11 ].val.ull = filetell( m_fp );					  // Spectrum Part Align
	...
	m_fileHeader[ 12 ].val.ull = filetell( m_fp );					 // Spectrum Property Align
	...
	m_fileHeader[ 13 ].val.ull = filetell( m_fp );					   // String Table Align

	progress.setStatus( "Writing string table." );
	m_fileHeader[ 14 ].val.ul = writeStringTable();				  // Compressed string table seize

	progress.setPosition( ++pos );	// pos == 7 + spectra

	m_fileHeader[ 15 ].val.ul = m_stringTable.size();				// Uncompressed String Table Size
	m_stringTable.clear();
