/**
 * @file MsbIO0104.cpp
 * @brief implements of MsbIO0104 class
 *
 * @author S.Tanaka
 * @date 2007.03.03
 * 
...
// static members
MsbManager::DataValue MsbIO0104::m_fileHeader[] = {
	...
	DataValue( TYPE_USHORT, (unsigned short)4 ),		//  3: Minor Version
	...
	DataValue( TYPE_ULONG,  (unsigned int)0 ),		 //  7: Spectrum Group Properties Align
	DataValue( TYPE_ULONG,  (unsigned int)0 ),		 //  8: Spectrum Data Align
	DataValue( TYPE_ULONG,  (unsigned int)0 ),		 //  9: Spectrum Align
	DataValue( TYPE_ULONG,  (unsigned int)0 ),		 // 10: Spectrum Part Align
	DataValue( TYPE_ULONG,  (unsigned int)0 ),		 // 11: Spectrum Properties Align
...
MsbManager::DataValue MsbIO0104::m_dataHeader[] = {
...
MsbManager::DataValue MsbIO0104::m_groupInfo[] = {
...
MsbManager::DataValue MsbIO0104::m_spectrumInfo[] = {
	...
	DataValue( TYPE_CHAR,   (char)0 ),			   // 13: Polarity
	DataValue( TYPE_STRING, (MsbString)0 ),		  // 14: Title
	DataValue( TYPE_BOOL,   (bool)false ),		   // 15: Chromatogram Flag
	DataValue( TYPE_STRING, (MsbString)0 )		   // 16: Icon Name
};

MsbManager::DataValue MsbIO0104::m_partInfo[] = {
...
MsbManager::DataValue MsbIO0104::m_prop[] = {
	DataValue( TYPE_ULONG,  (unsigned int)0 ),	  //  0: Spectrum
...
// create string table
void MsbIO0104::createStringTable( char** stringTable ) {
	// get size
...
void MsbIO0104::getFileHeaderInfo( MsbSample& sample, char* stringTable ) {
	// add file info
...
// read spectrum group
void MsbIO0104::readDataGroupNodes(
...
void MsbIO0104::readDataGroupNodeProperties(
		std::map< unsigned int, kome::objects::DataGroupNode* >& groupMap,
		char* stringTable
) {
   // read header
...
void MsbIO0104::readSpectra(
	...
	for( unsigned int i = 0; i < length; i++ ) {
		...
		char polarity = m_spectrumInfo[ 13 ].val.c;
		if( polarity < 0 ) {
		...
		spec->setTitle( stringTable + m_spectrumInfo[ 14 ].val.ms );
		spec->setHasChromatogram( m_spectrumInfo[ 15 ].val.b );
		spec->setIcon( stringTable + m_spectrumInfo[ 16 ].val.ms );
...
// read spectrum part
void MsbIO0104::readSpectrumParts(
		std::map< unsigned int, MsbSpectrum* >& specMap
){
	// read header
	READ_FROM_FILE( m_dataHeader );
...
void MsbIO0104::readSpectrumProperties(
		std::map< unsigned int, MsbSpectrum* >& specMap,
		char* stringTable
...
void MsbIO0104::writeDataGroupNodes(
...
void MsbIO0104::writeDataGroupNodeProperties(
		std::vector< kome::objects::DataGroupNode* >& groups,
		std::map< kome::objects::DataGroupNode*, unsigned int >& groupMap
) {
	// get properties
...
// write spectrum data
void MsbIO0104::writeSpectrumData(
		unsigned int id,
	...
	for( unsigned int i = 0; i < num; i++ ) {
		...
		// double
		float supX = arr.getPart( i )->getMz() + m_partRange / 2.0f;

		part->spectrum	   = id;
		part->length		 = arr.getPart( i )->getLength();
		part->start		  = arr.getPart( i )->getMinMz();
		part->end			= arr.getPart( i )->getMaxMz();
		part->totalIntensity = arr.getPart( i )->getTotalIntensity();
		part->maxIntensity   = arr.getPart( i )->getMaxIntensity();
		part->msAlign		= xPos + sizeof( float ) * startIdx;
		part->intAlign	   = yPos + sizeof( float ) * startIdx;

		startIdx += arr.getPart( i )->getLength();
	}
}

// write spectra
void MsbIO0104::writeSpectra(
	...
	for( unsigned int i = 0; i < dataSet.getNumberOfSpectra(); i++ ) {
		...
		if( spec->getPolarity() == kome::objects::Spectrum::POLARITY_NEGATIVE ) {
			m_spectrumInfo[ 13 ].val.c = -1;
		}
		else if( spec->getPolarity() == kome::objects::Spectrum::POLARITY_POSITIVE ) {
			m_spectrumInfo[ 13 ].val.c = 1;
		}
		else {
			m_spectrumInfo[ 13 ].val.c = 0;
		}																	   // Polarity
		m_spectrumInfo[ 14 ].val.ms  = st( spec->getTitle() );				  // Title
		m_spectrumInfo[ 15 ].val.b   = spec->hasChromatogram();				 // Chromatogram Flag
		m_spectrumInfo[ 16 ].val.ms  = st( spec->getIcon() );				   // Icon Name
...
void MsbIO0104::writeSpectrumParts( std::vector< SpectrumPartInfo >& parts ) {
...
void MsbIO0104::writeSpectrumProperties(
		kome::objects::DataSet& dataSet,
		std::map< kome::objects::Spectrum*, unsigned int >& spectrumMap
) {
	// get properties
...
void MsbIO0104::onReadSampleInfo( MsbSample& sample ) {
	// read file header
...
// open sample
void MsbIO0104::onReadMsb( MsbSample& sample ) {
	// create string table
	char* stringTable = NULL;
	createStringTable( &stringTable );
	...
	unsigned int specGroupPropAlign = m_fileHeader[ 7 ].val.ul;
	unsigned int spectrumAlign	  = m_fileHeader[ 9 ].val.ul;
	unsigned int specPartAlign	  = m_fileHeader[ 10 ].val.ul;
	unsigned int specPropAlign	  = m_fileHeader[ 11 ].val.ul;
	...
	// read spectrum group properties
	fseek( m_fp, specGroupPropAlign, SEEK_SET );
...
bool MsbIO0104::onWriteMsb(
	...
	// log
	LOG_INFO( FMT( "Writing MSB format sample. [ver 1.4]" ) );
	...
	m_fileHeader[ 7 ].val.ul = ftell( m_fp );
	...
	m_fileHeader[ 8 ].val.ul = ftell( m_fp );				   // Spectrum Data Align
	...
	m_fileHeader[ 9 ].val.ul = ftell( m_fp );					  // Spectrum Align
	...
	m_fileHeader[ 10 ].val.ul = ftell( m_fp );					  // Spectrum Part Align
	...
	m_fileHeader[ 11 ].val.ul = ftell( m_fp );					 // Spectrum Property Align
