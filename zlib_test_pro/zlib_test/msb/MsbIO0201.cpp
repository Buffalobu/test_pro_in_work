/**
 * @file MsbIO0201.cpp
 * @brief implements of MsbIO0201 class
 *
 * @author S.Tanaka
 * @date 2009.09.07
 * 
...
// static members
MsbManager::DataValue MsbIO0201::m_fileHeader[] = {
	...
	DataValue( TYPE_USHORT, (unsigned short)1 ),				 //  3: Minor Version
	...
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 12: Chromatogram Data Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 13: Chromatogram Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 14: Chromatogram Properties Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 15: Spectrum Data Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 16: Spectrum Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 17: Spectrum Part Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 18: Spectrum Properties Align
	DataValue( TYPE_ULONGLONG,  (unsigned long long)0 ),		 // 19: String Table Align
	DataValue( TYPE_ULONG,  (unsigned int)0 ),				  // 20: Compressed String Table Size
	DataValue( TYPE_ULONG,  (unsigned int)0 ),				  // 21: Uncompressed String Table Size
	DataValue( TYPE_TIME,   (time_t)0 )						  // 22: Time
};

MsbManager::DataValue MsbIO0201::m_dataHeader[] = {
...
MsbManager::DataValue MsbIO0201::m_groupInfo[] = {
...
MsbManager::DataValue MsbIO0201::m_chromatogramInfo[] = {
...
MsbManager::DataValue MsbIO0201::m_spectrumInfo[] = {
...
MsbManager::DataValue MsbIO0201::m_partInfo[] = {
...
MsbManager::DataValue MsbIO0201::m_prop[] = {
	DataValue( TYPE_ULONG,  (unsigned int)0 ),	  //  0: Spectrum
...
// create string table
void MsbIO0201::createStringTable( char** stringTable ) {
	// get size
	unsigned long stringTableSize = m_fileHeader[ 21 ].val.ul;
	...
	*stringTable = st;
	...
	// create buff
	unsigned int buffSize = m_fileHeader[ 20 ].val.ul;
	unsigned char* buff = new unsigned char[ buffSize ];

	// read string table
	fileseek( m_fp, m_fileHeader[ 19 ].val.ull, SEEK_SET );
...
void MsbIO0201::getFileHeaderInfo( MsbSample& sample, char* stringTable ) {
	// add file info
	...
	sample.setSoftwareName( stringTable + m_fileHeader[ 9 ].val.ul );
}

// read spectrum group
void MsbIO0201::readDataGroupNodes(
...
void MsbIO0201::readDataGroupNodeProperties(
		std::map< unsigned int, kome::objects::DataGroupNode* >& groupMap,
		char* stringTable
) {
   // read header
...
// read chromatograms
void MsbIO0201::readChromatograms(
		MsbSample& sample,
...
void MsbIO0201::readChromatogramProperties(
		std::map< unsigned int, MsbChromatogram* >& chromMap,
		char* stringTable
...
// read spectrum part
void MsbIO0201::readSpectrumParts(
		std::map< unsigned int, MsbSpectrum* >& specMap
){
	// read header
	READ_FROM_FILE( m_dataHeader );
...
void MsbIO0201::readSpectrumProperties(
		std::map< unsigned int, MsbSpectrum* >& specMap,
		char* stringTable
...
void MsbIO0201::writeDataGroupNodes(
...
void MsbIO0201::writeDataGroupNodeProperties(
		std::vector< kome::objects::DataGroupNode* >& groups,
		std::map< kome::objects::DataGroupNode*, unsigned int >& groupMap
) {
	// get properties
...
void MsbIO0201::writeChromatogramData(
		unsigned int id,
...
void MsbIO0201::writeChromatograms(
		std::vector< ChromatogramDataInfo >& chromData,
...
// write chromatogram
void MsbIO0201::writeChromatogramProperties(
...
// write spectrum data
int MsbIO0201::writeSpectrumData(
		unsigned int id,
...
void MsbIO0201::writeSpectrumParts( const int cnt, FILE* partsFp ) {
...
void MsbIO0201::writeSpectrumProperties(
		kome::objects::DataSet& dataSet,
		std::map< kome::objects::Spectrum*, unsigned int >& spectrumMap
) {
	// get properties
...
void MsbIO0201::onReadSampleInfo( MsbSample& sample ) {
	// read file header
...
// open sample
void MsbIO0201::onReadMsb( MsbSample& sample ) {
	// create string table
	char* stringTable = NULL;
	createStringTable( &stringTable );

	// keep align
	unsigned long long specGroupAlign	 = m_fileHeader[ 10 ].val.ull;
	unsigned long long specGroupPropAlign = m_fileHeader[ 11 ].val.ull;
	unsigned long long chromAlign		 = m_fileHeader[ 13 ].val.ull;
	unsigned long long chromPropAlign	 = m_fileHeader[ 14 ].val.ull;
	unsigned long long spectrumAlign	  = m_fileHeader[ 16 ].val.ull;
	unsigned long long specPartAlign	  = m_fileHeader[ 17 ].val.ull;
	unsigned long long specPropAlign	  = m_fileHeader[ 18 ].val.ull;
...
bool MsbIO0201::onWriteMsb(
	...
	// log
	LOG_INFO( FMT( "Writing MSB format sample. [ver 2.1]" ) );
	...
	m_fileHeader[ 22 ].val.t = time( NULL );									   // Time
	...
	m_fileHeader[ 10 ].val.ull = filetell( m_fp );				   // Spectrum Group Align
	...
	m_fileHeader[ 11 ].val.ull = filetell( m_fp );
	...
	m_fileHeader[ 12 ].val.ull = filetell( m_fp );   // Chromatogram Data Align
	...
	m_fileHeader[ 13 ].val.ull = filetell( m_fp );					  // Chromatograms Align
	...
	m_fileHeader[ 14 ].val.ull = filetell( m_fp );					 // Chromatogram Properties Align
	...
	m_fileHeader[ 15 ].val.ull = filetell( m_fp );   // Spectrum Data Align
	...
	m_fileHeader[ 16 ].val.ull = filetell( m_fp );					  // Spectrum Align
	...
	m_fileHeader[ 17 ].val.ull = filetell( m_fp );					  // Spectrum Part Align
	...
	m_fileHeader[ 18 ].val.ull = filetell( m_fp );					 // Spectrum Property Align
	...
	m_fileHeader[ 19 ].val.ull = filetell( m_fp );					   // String Table Align

	progress.setStatus( "Writing string table." );
	m_fileHeader[ 20 ].val.ul = writeStringTable();				  // Compressed string table seize

	progress.setPosition( ++pos );	// pos == 9 + chromatograms + spectra

	m_fileHeader[ 21 ].val.ul = m_stringTable.size();				// Uncompressed String Table Size
	m_stringTable.clear();
